...
def makeCsv(df):
 ...
 df.to_csv("IWantedATeammateButNoOneAskedAndImTooShy_2.csv", index=False)
...
def makeDf(device, model):
  ...
  for i in range(int(len(answers)/3)):
    final.append(getResult(answers[i*3], answers[i*3+1], answers[i*3+2]))
  df = pd.DataFrame({'Image_Name': _image_name, "Value":final})
  return df
